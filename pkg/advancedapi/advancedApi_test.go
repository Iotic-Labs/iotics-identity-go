...
func Test_can_create_new_registered_identity(t *testing.T) {
	resolver := test.NewInMemoryResolver()
	regID, regDoc, err := advancedapi.RegisterNewIdentity(resolver, identity.User, test.ValidKeyPairPlop, "#NewId", false)
	assert.NilError(t, err)
	assert.DeepEqual(t, regID.KeyPair(), test.ValidKeyPairPlop)
	assert.Equal(t, regID.Issuer().Name, "#NewId")

	assert.Assert(t, regDoc != nil)
	assert.Assert(t, regDoc.ID == regID.Did())
...
func Test_can_create_new_registered_identity_with_default_issuer_name(t *testing.T) {
	...
	for _, c := range cases {
		resolver := test.NewInMemoryResolver()
		regID, regDoc, err := advancedapi.RegisterNewIdentity(resolver, c.purpose, test.ValidKeyPairPlop, "", false)
		assert.NilError(t, err)
		assert.DeepEqual(t, regID.KeyPair(), test.ValidKeyPairPlop)
...
func Test_can_create_new_registered_identity_will_not_override_doc_if_exists(t *testing.T) {
	resolver := test.NewInMemoryResolver()
	_, doc, err := advancedapi.RegisterNewIdentity(resolver, identity.User, test.ValidKeyPairPlop, "#ExistingId", false)
	assert.NilError(t, err)

	assert.Check(t, len(doc.PublicKeys) == 1)
	assert.Check(t, doc.PublicKeys[0].ID == "#ExistingId")

	_, doc, err = advancedapi.RegisterNewIdentity(resolver, identity.User, test.ValidKeyPairPlop, "#NewId", false)
	assert.NilError(t, err)
...
func Test_can_create_new_registered_identity_will_override_doc_if_exists_and_override_true(t *testing.T) {
	...
	_, doc, err = advancedapi.RegisterNewIdentity(resolver, identity.User, test.ValidKeyPairPlop, "#NewId", true)
	assert.NilError(t, err)
...
func Test_can_delegate_authentication(t *testing.T) {
	resolver := test.NewInMemoryResolver()
	userID, userDoc, err := advancedapi.RegisterNewIdentity(resolver, identity.User, test.ValidKeyPairPlop, "#ExistingId", false)
	assert.NilError(t, err)
	agentID, agentDoc, err := advancedapi.RegisterNewIdentity(resolver, identity.User, test.ValidKeyPairPlop2, "#ExistingId", false)
	assert.NilError(t, err)
...
func Test_can_delegate_control(t *testing.T) {
	resolver := test.NewInMemoryResolver()
	twinID, twinDoc, err := advancedapi.RegisterNewIdentity(resolver, identity.Twin, test.ValidKeyPairPlop, "#ExistingId", false)
	assert.NilError(t, err)
	agentID, agentDoc, err := advancedapi.RegisterNewIdentity(resolver, identity.User, test.ValidKeyPairPlop2, "#ExistingId", false)
	assert.NilError(t, err)
...
func Test_can_add_auth_key_to_a_document(t *testing.T) {
	resolver := test.NewInMemoryResolver()
	twinID, _, err := advancedapi.RegisterNewIdentity(resolver, identity.Twin, test.ValidKeyPairPlop, "#ExistingId", false)
	assert.NilError(t, err)
...
func Test_can_validate_document(t *testing.T) {
	...
	otherId, _, err := advancedapi.RegisterNewIdentity(resolver, identity.Agent, test.ValidKeyPairPlop2, "#otherId", false)
	assert.NilError(t, err)
...
func Test_can_validate_document_delegations(t *testing.T) {
	resolver := test.NewInMemoryResolver()
	agentID, _, err := advancedapi.RegisterNewIdentity(resolver, identity.Agent, test.ValidKeyPairPlop, "#agent", false)
	assert.NilError(t, err)
	userID, _, err := advancedapi.RegisterNewIdentity(resolver, identity.User, test.ValidKeyPairPlop2, "#user", false)
	assert.NilError(t, err)
...
func Test_can_set_document_controller(t *testing.T) {
	resolver := test.NewInMemoryResolver()
	twinID, initialDoc, err := advancedapi.RegisterNewIdentity(resolver, identity.Twin, test.ValidKeyPairPlop, "#ExistingId", false)
	assert.NilError(t, err)
...
func Test_can_create_twin_auth_token(t *testing.T) {
	resolver := test.NewInMemoryResolver()
	twinID, _, err := advancedapi.RegisterNewIdentity(resolver, identity.Twin, test.ValidKeyPairPlop2, "#twin", false)
	assert.NilError(t, err)
...
func Test_can_validate_document_proof(t *testing.T) {
	resolver := test.NewInMemoryResolver()
	_, twinDoc, err := advancedapi.RegisterNewIdentity(resolver, identity.Twin, test.ValidKeyPairPlop, "#ExistingId", false)
	assert.NilError(t, err)
